...
__global__ void sigmoid_f32x4_kernel(float*x, float* y, int N){
    ...
    // 对于 exp 的操作 一般需要将数值限制在 FP32 或者 FP16 所能表示的范围内。
    reg_x.x = fminf(fmaxf(reg_x.x, MIN_EXP_F32), MAX_EXP_F32); // 将 reg_x 限制在 不小于 MIN_EXP_F32, 避免 小于 MIN_EXP_F32 时直接变为零，导致精度不足。
    reg_x.y = fminf(fmaxf(reg_x.y, MIN_EXP_F32), MAX_EXP_F32); // 将 reg_x 限制在 不大于 MAX_EXP_F32, 避免 大于 MAX_EXP_F32 时直接变为无穷大，导致精度不足。
...
__global__ void sigmoid_f16_kernel(half* x, half* y, int N) {
    ...
    if(idx < N){
        half v = x[idx];
        v = __hmin(__hmax(v, MIN_EXP_F16), MAX_EXP_F16); // 防止溢出 或者 不稳定的计算
        y[idx] = f / (f + __hmul(f, hexp(-v)));
    }
}
// hexp 是专门用于 FP16（半精度浮点数，half 类型） 的指数函数。
// expf 是用于 FP32（单精度浮点数，float 类型） 的指数函数。
// __hmin , __hmax FP16 ， fmaxf fminf 同理
__global__ void sigmoid_f16x2_kernel(half* x, half* y, int N){
    ...
    reg_y.x = __hmin(__hmax(reg_y.x, MIN_EXP_F16), MAX_EXP_F16);
...
__global__ void sigmoid_f16x8_kernel(half* x, half* y, int N){
    ...
    if((idx + 0) < N){ HALF2(y[idx]) = reg_y_0; }
...
__global__ void sigmoid_f16x8_pack_kernel(half* x, half* y, int N){
    int idx = (blockIdx.x * blockDim.x + threadIdx.x) * 8;
    const half f = __float2half(1.0f);
    // 创建临时 register, 实际是在 ptx 中是 local memory,其实是列表形式
    half pack_x[8];
    half pack_y[8];

    // 加载 128 位数据到 pack_x 中 float4
